    ...
    return (
        <div>
            ...
            <p className="page-lede">
                Crude producers currently under contract or evaluation. Company, country, and capacity
                are reference data; cost/bbl is recomputed live from the EIA Brent spot price on every
                refresh. Risk baseline is a static classification — Live risk is derived from this
                supplier's route(s), recomputed in real time from GDELT news + Gemini event scoring.
            </p>
                    ...
                    <table>
                        ...
                        {suppliers?.map((s) => (
                            <tr key={s.id}>
                                ...
                                <td>
                                    <RiskGauge value={Math.round((s.liveRiskScore || 0) * 10)} max={10} size={44} />
                                    <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 2 }}>
                                        {s.riskSource === "ROUTE_DERIVED" ? "from route events" : "no route data"}
                                    </div>
                                </td>
                            </tr>
                        ))}
