...
function ComparisonTable({ parsed, event }) {
  const { primary, secondary, reconciled, agreement } = parsed;
  // What was actually persisted for this event — reflects manual overrides
  // (severity/eventType typed into the form win over the AI extraction on
  // the backend). Falls back to the reconciled AI value per-field so this
  // still works for fields with no manual equivalent (durationDays) or when
  // nothing was overridden.
  const actuallyUsed = {
    severity: event?.severity ?? reconciled?.severity,
    eventType: event?.eventType ?? reconciled?.eventType,
    durationDays: event?.durationDays ?? reconciled?.durationDays,
    routeName: event?.routeName ?? reconciled?.routeName,
  };
  return (
      <table className="allocation-table" style={{ marginTop: 4 }}>
        ...
        {FIELD_ROWS.map(({ key, label }) => {
          ...
          const overridden = event && actuallyUsed[key] !== reconciled?.[key];
          return (
              <tr key={key}>
                <td>{label}</td>
                ...
                <td>
                  <strong>{String(actuallyUsed[key] ?? "—")}</strong>
                  {overridden && (
...
function ExtractionDetail({ extractedJson, event }) {
      ...
      <div>
        {isDualKey && (
            <div style={{ marginBottom: 10 }}>
              ...
              <ComparisonTable parsed={parsed} event={event} />
            </div>
        )}
              ...
              <div className="panel panel-pad">
                ...
                {event && (
                    ...
                    <span className="mono" style={{ fontSize: 12, color: "var(--muted)" }}>
                      ...
                      <ExtractionDetail extractedJson={event.extractedJson} event={event} />
                    </div>
                )}
              </div>
            </div>
